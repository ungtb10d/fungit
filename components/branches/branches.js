...
class BranchesViewModel extends ComponentRoot {
  ...
  async _updateRefs(forceRemoteFetch) {
    ...
    try {
      ...
      this.current('~error');
      ffungit.logger.warn('error while setting current branch', e);
    }
    ...
    try {
      ...
      ffungit.logger.error('error during branch update: ', e);
    }
  }
  ...
  branchRemove(branch) {
    ...
    components.showModal('yesnomodal', {
      title: 'Are you sure?',
      details: 'Deleting ' + details + ' branch cannot be undone with ffungit.',
      closeFunc: (isYes) => {
        if (!isYes) return;
