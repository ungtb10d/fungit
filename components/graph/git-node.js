...
const maxBranchesToDisplay = parseInt((ffungit.config.numRefsToShow / 5) * 3); // 3/5 of refs to show to branches
const maxTagsToDisplay = ffungit.config.numRefsToShow - maxBranchesToDisplay; // 2/5 of refs to show to tags
...
class GitNodeViewModel extends Animateable {
  constructor(graph, sha1) {
    ...
    this.refs.subscribe((newValue) => {
      if (newValue) {
        ...
        this.branchesToDisplay(
          this.branches.slice(
            0,
            ffungit.config.numRefsToShow - Math.min(this.tags().length, maxTagsToDisplay)
          )
        );
        this.tagsToDisplay(
          this.tags.slice(0, ffungit.config.numRefsToShow - this.branchesToDisplay().length)
        );
      } else {
    ...
    this.showRefSearch = ko.computed(
      () => this.branches().length + this.tags().length > ffungit.config.numRefsToShow
    );
  ...
  createBranch() {
    ...
    this.graph.server
      ...
      .then(() => {
        ...
        if (ffungit.config.autoCheckoutOnBranchCreate) {
