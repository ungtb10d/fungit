  ...
  perform() {
    ...
    return new Promise((resolve, reject) => {
      components.showModal('yesnomodal', {
        title: 'Are you sure?',
        details: 'Resetting to ref: ' + remoteRef.name + ' cannot be undone with ffungit.',
...
class Rebase extends ActionBase {
  constructor(graph, node) {
    ...
    this.visible = ko.computed(() => {
      ...
      return (
        this.graph.currentActionContext() instanceof RefViewModel &&
        (!ffungit.config.showRebaseAndMergeOnlyOnRefs || this.node.refs().length > 0) &&
    ...
    return this.server
      ...
      .catch((err) => {
        if (err.errorCode != 'merge-failed') {
          ...
          ffungit.logger.warn('rebase failed', err);
        }
      });
  }
}
    ...
    return this.server
      ...
      .catch((err) => {
        if (err.errorCode != 'merge-failed') {
          ...
          ffungit.logger.warn('merge failed', err);
        }
      });
  }
}
...
class Checkout extends ActionBase {
  constructor(graph, node) {
    ...
    this.visible = ko.computed(() => {
      ...
      return ffungit.config.allowCheckoutNodes && this.graph.currentActionContext() == this.node;
    });
  }
...
class Delete extends ActionBase {
  ...
  perform() {
    ...
    details = `Deleting ${details} branch or tag cannot be undone with ffungit.`;
    ...
    return this.server
      ...
      .catch((err) => {
        if (err.errorCode != 'merge-failed') {
          ...
          ffungit.logger.warn('cherrypick failed', err);
        }
      });
  }
}
