...
const numberOfNodesPerLoad = ffungit.config.numberOfNodesPerLoad;
...
class GraphViewModel extends ComponentRoot {
  ...
  async _loadNodesFromApi() {
    ...
    ffungit.logger.debug('graph.loadNodesFromApi() triggered');
    ...
    try {
      ...
      this._isLoadNodesFromApiRunning = false;
      ffungit.logger.debug('graph.loadNodesFromApi() finished');
    }
  }
  ...
  async _updateBranches() {
    ...
    try {
      ffungit.logger.debug('setting checkedOutBranch', checkout);
      this.checkedOutBranch(checkout);
    } catch (err) {
      if (err.errorCode != 'not-a-repository') {
        this.server.unhandledRejection(err);
      } else {
        ffungit.logger.warn('updateBranches failed', err);
      }
    }
  }
