...
class AppViewModel {
  ...
  shown() {
    ...
    if (!fungit.config.bugtracking) {
      // Whereas the userconfig only reflects what's in the ~/.fungitrc and updates directly,
    ...
    this.server
      ...
      .then((version) => {
        ...
        this.showNewVersionAvailable(!fungit.config.fungitVersionCheckOverride && version.outdated);
      })
      .catch((e) => this.server.unhandledRejection(e));
    this.server
