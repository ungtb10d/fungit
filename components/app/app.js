...
class AppViewModel {
  constructor(appContainer, server) {
    ...
    this.newVersionInstallCommand =
      (ffungit.platform == 'win32' ? '' : 'sudo -H ') + 'npm update -g ffungit';
    this.bugtrackingEnabled = ko.observable(ffungit.config.bugtracking);
    ...
    this.gitVersionErrorVisible = ko.computed(() => {
      return (
        !ffungit.config.gitVersionCheckOverride &&
  ...
  getRepoList() {
    ...
    const newRepos = localStorageRepo
      .concat(ffungit.config.defaultRepositories || [])
  ...
  shown() {
    // The ffungit.config constiable collections configuration from all different paths and only updates when
    // ffungit is restarted
    if (!ffungit.config.bugtracking) {
      // Whereas the userconfig only reflects what's in the ~/.ffungitrc and updates directly,
      // but is only used for changing around the configuration. We need to check this here
      // since ffungit may have crashed without the server crashing since we enabled bugtracking,
    ...
    this.server
      ...
      .then((version) => {
        ...
        this.showNewVersionAvailable(!ffungit.config.ffungitVersionCheckOverride && version.outdated);
      })
      .catch((e) => this.server.unhandledRejection(e));
    this.server
