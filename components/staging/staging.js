...
const mergeTool = ffungit.config.mergeTool;
...
class StagingViewModel extends ComponentRoot {
  ...
  async _refreshContent() {
    ffungit.logger.debug('staging.refreshContent() triggered');
    ...
    try {
      ...
      if (Object.keys(status.files).length > filesToDisplayLimit && !this.loadAnyway) {
        ...
        components.showModal('toomanyfilesmodal', {
          ...
          details: 'It is recommended to use command line as ffungit may be too slow.',
          closeFunc: (isYes) => {
            this.isDiagOpen = false;
            if (isYes) {
      ...
      if (err.errorCode != 'must-be-in-working-tree' && err.errorCode != 'no-such-path') {
        ...
        ffungit.logger.error('error during staging refresh: ', err);
      }
    } finally {
      ffungit.logger.debug('staging.refreshContent() finished');
    }
  }
...
class FileViewModel {
  ...
  discardChanges() {
    ...
    const isMuteWarning = timeSinceLastMute < ffungit.config.disableDiscardMuteTime;
    ffungit.logger.debug(
      `discard time since mute: ${timeSinceLastMute}, isMuteWarning: ${isMuteWarning}`
    );
    if (ffungit.config.disableDiscardWarning || isMuteWarning) {
      this.server
