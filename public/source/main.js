...
ffungit.logger = winston.createLogger({
  level: ffungit.config.logLevel || 'error',
...
var navigation = require('ffungit-navigation');
var storage = require('ffungit-storage');
var adBlocker = require('just-detect-adblock');
var { encodePath } = require('ffungit-address-parser');
...
ko.bindingHandlers.autocomplete = {
  init: (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) => {
    ...
    const handleKeyEvent = (event) => {
      ...
      if (lastChar == ffungit.config.fileSeparator) {
        ...
        server
          ...
          .then((directoryList) => {
            ...
            $(element).val(
              currentDir.endsWith(ffungit.config.fileSeparator)
                ? currentDir
                : currentDir + ffungit.config.fileSeparator
            );
        ...
        const folderNames = JSON.parse(storage.getItem('repositories')).map((value) => {
          return {
            value: value,
            label: value.substring(value.lastIndexOf(ffungit.config.fileSeparator) + 1),
          };
        });
...
WindowTitle.prototype.update = function () {
  var title = this.path
    ...
    .reverse()
    .join(' < ');
  if (this.crash) title = ':( ffungit crash ' + title;
...
exports.start = function () {
  ...
  if (ffungit.config.authentication) {
