...
module.exports = async ({ github, context, core, exec }) => {
  ...
  const release = await github.rest.repos.createRelease({
    ...
    name: tag,
    tag_name: tag,
    body: `[Changelog](https://github.com/FredrikNoren/ffungit/blob/master/CHANGELOG.md#${version.replace(
      /\./g,
      ''
    )})`,
  });
  ...
  for (let i = 0; i < filePaths.length; i++) {
    ...
    await github.rest.repos.uploadReleaseAsset({
      ...
      name: path.basename(filePath).replace('ffungit', `ffungit-${version}`),
